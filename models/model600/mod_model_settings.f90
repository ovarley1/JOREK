...
! #SETTINGS# with_vpar with_TiTe with_rho with_neutrals with_impurities
...
integer, parameter :: i_ext_rho            = 3
integer, parameter :: i_ext_neutrals       = 4
integer, parameter :: i_ext_impurities     = 5
integer, parameter :: i_ext_refluid        = 6
logical, parameter :: with_ext(n_mod_ext) = &
  (/ with_TiTe, with_vpar, with_rho, with_neutrals, with_impurities, with_refluid /)

! --- number of variables (for base model, extension, and in total)
integer, parameter :: n_var_base        = 5         !< number of variables in base model
integer, parameter :: n_var_TiTe        = sum(merge( (/1/), (/0/), with_TiTe      ))
integer, parameter :: n_var_vpar        = sum(merge( (/1/), (/0/), with_vpar      ))
integer, parameter :: n_var_rho         = sum(merge( (/1/), (/0/), with_rho       ))
...
integer, parameter :: var_rho  = sum(merge( (/5/), (/0/), with_rho ))
integer, parameter :: var_T    = sum(merge( (/0/), (/n_var_base+n_var_rho/), with_TiTe ))
! --- variable indices for the model extensions
integer, parameter :: var_Ti       = sum(merge((/n_var_base+n_var_rho             /), (/0/), with_TiTe      ))
integer, parameter :: var_Te       = sum(merge((/n_var_base+n_var_rho+n_var_vpar+1/), (/0/), with_TiTe      ))
integer, parameter :: var_Vpar     = sum(merge((/n_var_base+n_var_rho+n_var_vpar  /), (/0/), with_vpar      ))
integer, parameter :: var_rhon     = sum(merge((/n_var_base+sum(n_var_ext(1:3))+1 /), (/0/), with_neutrals  ))
integer, parameter :: var_rhoimp   = sum(merge((/n_var_base+sum(n_var_ext(1:4))+1 /), (/0/), with_impurities))
integer, parameter :: var_nre      = sum(merge((/n_var_base+sum(n_var_ext(1:5))+1 /), (/0/), with_refluid   ))
