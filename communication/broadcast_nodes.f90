...
#ifndef USE_DOMM
! bufsize = node_list%n_nodes * ((n_coord_tor*n_degrees*(n_dim+2*3+1+1+2*(n_dim+1)) + 2*n_tor*n_degrees*n_var + n_tor*n_degrees + 2 + 2*n_degrees)*IDBL_EXT + (n_degrees + 1+3+1+1)*INT_EXT + (2)*ILOG_EXT)
...
bufsize = node_list%n_nodes * ((n_coord_tor*n_degrees*(n_dim+2*(n_dim+1)) + 2*n_tor*n_degrees*n_var + n_tor*n_degrees + 2 + 2*n_degrees)*IDBL_EXT + (n_degrees + 1+3+1+1)*INT_EXT + (2)*ILOG_EXT)
#endif
#elif fullmhd
