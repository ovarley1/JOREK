...
use mod_fields !,  only : check_consistency_Qin, check_consistency_RK4
...
real*8,allocatable :: error_W_rk4(:), error_W_rk2(:), error_W_qin(:), error_W_lf(:)
real*8,allocatable :: error_P_rk4(:), error_P_rk2(:), error_P_qin(:), error_P_lf(:)
integer   :: i_elm_out
!$ real*8 :: w0, w1, mmm(3)

integer   :: ifail, n_part_phi, n_particles_local
integer   :: i, j, n_steps, n_phases, in
integer   :: seed, i_rng, n_stream, ierr, n_particle_out
character*14 :: fileout, filepart
type(particle_gc_vpar) :: p_gc, p_check_gc, p_gc2
type(particle_gc)      :: p_Emu
type(particle_kinetic_leapfrog), allocatable :: p_orbit(:)

call sim%initialize(num_groups=4)
...
allocate(error_W_rk4(nstep), error_W_rk2(nstep), error_W_qin(nstep), error_W_lf(nstep))
allocate(error_P_rk4(nstep), error_P_rk2(nstep), error_P_qin(nstep), error_P_lf(nstep))
...
sim%groups(4)%Z    = 1
sim%groups(4)%mass = atomic_weights(1) !< atomic mass units, -2 for deuterium
...
allocate(particle_gc_vpar::sim%groups(4)%particles(n_particles_local))

allocate(p_orbit(n_phases))

call sim%fields%check_consistency_Qin(sim%fields%element_list%n_elements/2, [0.25, 0.5])
call sim%fields%check_consistency_RK4(sim%fields%element_list%n_elements/2, [0.25, 0.5])
...
!    call sim%fields%calc_RK4(sim%time, p_gc2(1)%i_elm, p_gc2(1)%st, p_gc2(1)%x(3), A, dA, B, dB, Bnorm, dBnorm, bn, dbn, E)
!    !call sim%fields%calc_RK4_analytic(p_gc(1)%x(1), p_gc(1)%x(2), p_gc(1)%x(3), A, dA, B, dB, Bnorm, dBnorm, bn, dbn, E)
...
!    write(112,'(A,i6,12e18.10)') 'RK2: ',p_gc2(1)%i_elm,p_gc2(1)%mu,p_gc2(1)%x,p_gc2(1)%vpar,(p_phi_gc2-p_phi_gc2_start)/p_phi_gc2_start,(energy_gc2-energy_gc2_start)/energy_gc2_start
...
!write(*,'(A,12e18.10)') 'RK2 : P_phi, energy : ',sim%time,  P_phi_gc2_start, energy_gc2_start,&
!                      P_phi_gc2, energy_gc2, (P_phi_gc2-P_phi_gc2_start)/P_phi_gc2_start,(energy_gc2-energy_gc2_start)/energy_gc2_start
...
!write(*,'(A,8e14.6)') 'RK2 max(error) : ',maxval(error_P_rk2),maxval(error_W_rk2),sum(error_P_rk2)/real(nstep,8),sum(error_W_rk2)/real(nstep,8)
